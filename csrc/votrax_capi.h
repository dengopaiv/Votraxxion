...
//
// This is the surface an NVDA add-on binds to: a handful of `extern "C"`
// entry points that ctypes can call without a C++ ABI, a compiler, or any
// knowledge of what is behind them.  Everything the synthesizer needs is
// compiled in — both mask ROMs and the whole English front end — so the
// add-on ships one library file and nothing else.
//
// Threading: a vx_chip is not internally locked.  The intended shape, and the
// one the reference add-on used, is that exactly one thread touches the chip
...
// Mask revisions, matching MaskRevision in rom_data.h.
...
// Queue phones for the scheduler.  Returns the number now queued.
...
// contour.
VOTRAX_API int ttv_translate_flat(const char *text, unsigned char *out, int capacity);
